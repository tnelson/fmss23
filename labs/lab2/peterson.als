/*
  FMSS23 Lab 2 (Lock 3: Peterson Lock)

  Dev note: finally, we reach a lock that should work. Let's check it, but also:
     * write some better validation together!
     * ...is there another "oops" in how we wrote the non-starvation property?
       * missing fairness = {A->raise; B->noThanks forever}
       * then double-check reference to flag rather than location
